...
public class DSquare {
	...
	private void execute(double[] left, double[] top){
		...
		while(dimension1 / div >= 2){
			int inc = (dimension1 - 1) / div;	
			for(int y = 0; y < dimension1; y += inc){
				if(inc + y >= dimension1) break;
				for(int x = 0; x < dimension1; x += inc){
					if(inc + x >= dimension1) break;
	...
	private double[] getLastColumn(){
		if(array != null){
			double[] buffer = new double[dimension1];
			for(int i = 0; i < dimension1; i++){
				buffer[i] = array[dimension1 - 1][i];
			}
			return buffer;
		}
		return null;
	...
	private void loadRandomValues(){
		...
		setIfEmpty(dimension1 - 1, 0, rng.nextDouble());
		setIfEmpty(0, dimension1 - 1, rng.nextDouble());
		setIfEmpty(dimension1 - 1, dimension1 - 1, rng.nextDouble());
	}
